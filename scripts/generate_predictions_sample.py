...
def generate():
    ...
    inputs = df.drop(columns=["a_quitte_l_entreprise", "departement"])

    features = _compute_features(inputs.copy())
    probas = pipeline.predict_proba(features)[:, 1]
    predictions = (probas >= threshold).astype(int)

    inputs["prediction"] = predictions
    inputs["probabilite"] = probas.round(4)
    inputs["label"] = ["Quitte" si p == 1 else "Reste" for p in predictions]
